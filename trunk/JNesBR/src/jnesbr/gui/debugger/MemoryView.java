...
public class MemoryView extends javax.swing.JFrame {
    ...
    private void jBtnGoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnGoActionPerformed
        ...
        if (userChoose.equals("LOWER BANK")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.LOWER_BANK_START,MemoryMap.LOWER_BANK_END));
            return;
        }
        if (userChoose.equals("UPPER BANK")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.UPPER_BANK_START,MemoryMap.UPPER_BANK_END));
            return;
        }
        if (userChoose.equals("ZERO PAGE")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.ZERO_PAGE_START,MemoryMap.ZERO_PAGE_END));
            return;
        }
        if (userChoose.equals("STACK")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.STACK_START,MemoryMap.STACK_END));
            return;
        }
        if (userChoose.equals("RAM")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.RAM_START,MemoryMap.RAM_END));
            return;
        }

        if (userChoose.equals("MIRROR ZERO PAGE")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_0_ZERO_PAGE_START1,MemoryMap.MIRROR_0_ZERO_PAGE_END1));
            return;
        }
        if (userChoose.equals("MIRROR STACK")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_0_STACK_START1,MemoryMap.MIRROR_0_STACK_END1));
            return;
        }
        if (userChoose.equals("MIRROR RAM")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_0_RAM_0_START1,MemoryMap.MIRROR_0_RAM_0_END1));
            return;
        }

        if (userChoose.equals("MIRROR 1 ZERO PAGE")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_1_ZERO_PAGE_START1,MemoryMap.MIRROR_1_ZERO_PAGE_END1));
            return;
        }
        if (userChoose.equals("MIRROR 1 STACK")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_1_STACK_START1,MemoryMap.MIRROR_1_STACK_END1));
            return;
        }
        if (userChoose.equals("MIRROR 1 RAM")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_1_RAM_0_START1,MemoryMap.MIRROR_1_RAM_0_END1));
            return;
        }

        if (userChoose.equals("MIRROR 2 ZERO PAGE")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_2_ZERO_PAGE_START1,MemoryMap.MIRROR_2_ZERO_PAGE_END1));
            return;
        }
        if (userChoose.equals("MIRROR 2 STACK")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_2_STACK_START1,MemoryMap.MIRROR_2_STACK_END1));
            return;
        }
        if (userChoose.equals("MIRROR 2 RAM")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.MIRROR_2_RAM_0_START1,MemoryMap.MIRROR_2_RAM_0_END1));
            return;
        }
        if (userChoose.equals("IO REGISTERS")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.IO_REGISTERS_START,MemoryMap.IO_REGISTERS_END));
            return;
        }

        if (userChoose.equals("EXPANSION ROM")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.EXPANSION_ROM_START,MemoryMap.EXPANSION_ROM_END));
            return;
        }

        if (userChoose.equals("SAVE RAM")){
            jTMemory.setModel(Emulator.getMemoryModel(jTMemory,MemoryMap.SAVE_RAM_START,MemoryMap.SAVE_RAM_END));
            return;
        }
